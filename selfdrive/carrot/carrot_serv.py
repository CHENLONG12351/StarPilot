...
class CarrotServ:
  ...
  def update_params(self):
    self.autoNaviSpeedBumpSpeed = float(self.params.get_int("AutoNaviSpeedBumpSpeed", default=30))
    self.autoNaviSpeedBumpTime = float(self.params.get_int("AutoNaviSpeedBumpTime", default=3))
    self.autoNaviSpeedCtrlEnd = float(self.params.get_int("AutoNaviSpeedCtrlEnd", default=5))
    self.autoNaviSpeedCtrlMode = self.params.get_int("AutoNaviSpeedCtrlMode")
    self.autoNaviSpeedSafetyFactor = float(self.params.get_int("AutoNaviSpeedSafetyFactor", default=100)) * 0.01
    self.autoNaviSpeedDecelRate = float(self.params.get_int("AutoNaviSpeedDecelRate", default=125)) * 0.01
    self.autoNaviCountDownMode = self.params.get_int("AutoNaviCountDownMode", default=1)
    self.turnSpeedControlMode= self.params.get_int("TurnSpeedControlMode")
    self.mapTurnSpeedFactor = self.params.get_float("MapTurnSpeedFactor", default=100.0) * 0.01

    self.autoTurnControlSpeedTurn = self.params.get_int("AutoTurnControlSpeedTurn", default=25)
    self.autoTurnMapChange = self.params.get_int("AutoTurnMapChange")
    self.autoTurnControl = self.params.get_int("AutoTurnControl")
    self.autoTurnControlTurnEnd = self.params.get_int("AutoTurnControlTurnEnd", default=5)
    #self.autoNaviSpeedDecelRate = float(self.params.get_int("AutoNaviSpeedDecelRate")) * 0.01
    self.autoCurveSpeedLowerLimit = self.params.get_int("AutoCurveSpeedLowerLimit", default=20)
    self.is_metric = self.params.get_bool("IsMetric")
    self.autoRoadSpeedLimitOffset = self.params.get_int("AutoRoadSpeedLimitOffset", default=-1)
  ...
  def update_navi(self, remote_ip, sm, pm, vturn_speed, coords, distances, route_speed, gps_service):
    ...
    if sm.alive['carState'] and sm.alive['selfdriveState']:
      ...
      v_ego = CS.vEgo
      v_ego_kph = v_ego * 3.6
      # SP: SelfdriveState has no distanceTraveled; integrate v_ego at the 20 Hz loop rate
      delta_dist = v_ego / 20.0
      self.totalDistance += delta_dist
      if getattr(CS, 'speedLimit', 0) > 0 and self.active_carrot <= 1:
        self.nRoadLimitSpeed = getattr(CS, 'speedLimit', 0)
    else:
      v_ego = v_ego_kph = 0
      delta_dist = 0
      CS = None
    ...
    if (self.xSpdDist > 0 or self.xSpdType in [100, 101]) and self.active_carrot > 0:
      ...
      sdi_speed = min(sdi_speed,
                      self.calculate_current_speed(getattr(CS, 'speedLimitDistance', 0),
                                                   getattr(CS, 'speedLimit', 0) * self.autoNaviSpeedSafetyFactor,
